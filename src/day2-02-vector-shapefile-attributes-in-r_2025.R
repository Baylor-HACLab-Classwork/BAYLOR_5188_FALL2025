#Title: day2-02-vector-shapefile-attributes-in-r_2025.R
...
#to identify and query shapefile attributes, as well as how to subset 
#shapefiles by specific attribute values. Finally, we will learn how 
#to plot a shapefile according to a set of attribute values.

## Load the Data
#We will continue using the `sf`, `raster` and `ggplot2` packages in 
#this episode. Make sure that you have these packages loaded. We will 
...
#As we discussed previously, we can view metadata associated 
#with an R object using:
...
#To see a summary of all of the metadata associated with our `point_HARV` 
#object, we can view the object with `View(point_HARV)` or print a summary 
#of the object itself to the console.

point_HARV


#We can use the `ncol` function to count the number of attributes 
#associated with a spatial object too. Note that the geometry is 
#just another column and counts towards the total.
...
#of attribute values using the `head()` function to get a 
#preview of the data:
...
#Explore the attributes associated with the `point_HARV` and 
#`aoi_boundary_HARV` spatial objects.

#1. How many attributes does each have?
#2. Who owns the site in the `point_HARV` data object?
#3. Which of the following is NOT an attribute of the `point_HARV` 
#data object?
...
#to exploring values in a column. We did this with the `gapminder` 
#dataframe in an earlier lesson. For spatial objects, we can use 
#the same syntax: `objectName$attributeName`.
...
#within R is factor. We worked with factors a little bit in an earlier lesson.
...
#an earlier lesson  to select a subset of features
#from a spatial object in R, just like with data frames.

#For example, we might be interested only in features that are of 
#`TYPE` "footpath". Once we subset out this data, we can use it as 
#input to other code so that code only operates on the footpath lines.
...
#there is only one feature? Let's adjust the colors used in our plot. 
#If we have 2 features in our vector object, we can plot each using a 
#unique color by assigning a column name to the color aesthetic (`color =`). 
#We use the syntax `aes(color = )` to do this. We can also alter the default 
#line thickness by using the `size =` parameter, as the default value of 
#0.5 can be hard to see. Note that size is placed outside of the `aes()` 
#function, as we are not connecting line thickness to a data variable.
...
#Subset out all `stone wall` features from the lines layer and plot it. 
...
### Adjust Line Width
#We adjusted line width universally earlier. If we want a unique line width 
#for each factor level or attribute category in our spatial object, we can 
#use the same syntax that we used for colors, above.

#We already know that we have four different `TYPE` levels in the lines_HARV 
#object, so we will set four different line widths.
...
#Note that we could also use `aes(size = TYPE)` to tie the line thickness 
#to the TYPE variable, so long as we had been careful to set factor levels 
#appropriately. ggplot prints a warning when you do this, because it is not 
#considered a good practice to plot non-spatial data this way.
...
ggplot() +
   ...
   ggtitle("NEON Harvard Forest Field Site", subtitle = "Roads & Trails - Line width varies") + coord_sf()



### Add Plot Legend

#We can add a legend to our plot too. When we add a legend, we use the 
#following elements to specify labels and colors:
...
ggplot() + 
  ...
  ggtitle("NEON Harvard Forest Field Site", subtitle = "Roads Where Bikes and Horses Are Allowed") + coord_sf()
...
# Clean & normalize geometry to polygons
state_boundary_US <- state_boundary_US |>
  st_as_sf() |>
  st_zm(drop = TRUE, what = "ZM") |>                # drop Z/M dims
  st_make_valid() |>
  st_collection_extract("POLYGON") |>               # pull polygons out of collections
  st_cast("MULTIPOLYGON", warn = FALSE)             # consistent type
...
colors <- c(West="purple", Northeast="springgreen", Southeast="yellow",
             Midwest="brown", Southwest="navy")
#Now we can create our plot: 
