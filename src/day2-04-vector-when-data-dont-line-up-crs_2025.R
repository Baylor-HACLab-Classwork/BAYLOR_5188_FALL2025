#Title: day2-04-vector-when-data-dont-line-up-crs_2025.R
...
#United States Census Bureau. We will learn how to map vector 
#data that are in different CRSs and thus don't line up on a map.
...
#that they are in the same projection to support plotting / mapping. Note that
#these skills are also required for any geoprocessing / spatial analysis. Data need
#to be in the same CRS to ensure accurate results.
...
#layer into R. This layer contains the boundaries of all contiguous 
#states in the U.S. Please note that these data have been modified 
...
#it will make our map pop! We will also manually set the colors of the 
#state boundaries and country boundaries.
...
#Our project string for state_boundary_US and country_boundary_U
...
#We saw in an earlier episode that when working with raster data in 
#different CRSs, we needed to convert all objects to the same
...
#1. Import and plot Boundary-US-State-NEast.shp. Adjust line 
#width as necessary.

#2. Layer the Fisher Tower (in the NEON Harvard Forest site) point 
#location point_HARV onto the plot.
...
# Clean & normalize geometry to polygons
NE.States.Boundary.US <- NE.States.Boundary.US |>
  st_as_sf() |>
  st_zm(drop = TRUE, what = "ZM") |>                # drop Z/M dims
  st_make_valid() |>
  st_cast("MULTIPOLYGON", warn = FALSE)             # consistent type
