#Title: day2-05-vector-csv-to-shapefile-r_2025.R
...
#We will also reproject data imported from a shapefile format, 
#export this data as a shapefile, and plot raster and vector 
#data as layers in the same plot.

## Spatial Data in Text Format

#The `HARV_PlotLocations.csv` file contains `x, y` (point) locations 
#for study plot where NEON collects data on vegetation and other 
#ecological metrics
...
#Spatial data are sometimes stored in a text file format (`.txt` or `.csv`). 
#If the text file has an associated `x` and `y` location column, 
#then we can convert it into an `sf` spatial object. The `sf` object 
#allows us to store both the `x,y` values that represent the coordinate 
#location of each point and the associated attribute data - or 
...
#We now have a data frame that contains 21 locations (rows) and 16 
...
#information. The `plot_locations_HARV$easting`
#and `plot_locations_HARV$northing` columns contain coordinate values. 
#We can confirm this by looking at the first six rows of our data.
...
#data.
#2. We can explore the file itself to see if CRS information is embedded 
#in the file header or somewhere in the data columns.

#Following the `easting` and `northing` columns, there is a `geodeticDa` 
#and a `utmZone` column. These appear to contain CRS information
...
#file contains CRS information this way. The `geodeticDa` and `utmZone` 
#columns contain the information that helps us determine the CRS:

#* `geodeticDa`: WGS84  -- this is geodetic datum WGS84
#* `utmZone`: 18

#In When Vector Data Don't Line Up - Handling Spatial Projection & CRS in R
#we learned about the components of a `proj4` string. We have everything
#we need to assign a CRS to our data frame.

#To create the `proj4` associated with UTM Zone 18 WGS84 we can look up the
#projection on the Spatial Reference website
#http://www.spatialreference.org/ref/epsg/wgs-84-utm-zone-18n/, 
#which contains a list of CRS formats for each projection. From here, 
#we can extract the proj4 string for UTM Zone 18N WGS84
#http://www.spatialreference.org/ref/epsg/wgs-84-utm-zone-18n/proj4/).

#However, if we have other data in the UTM Zone 18N projection, it's much
#easier to use the `st_crs()` function to extract the CRS in `proj4` 
#format from that object and assign it to our new spatial object. 
...
#In Open and Plot Shapefiles in R
#we learned about spatial object extent. When we plot several spatial layers 
#in R using `ggplot`, all of the layers of the plot are considered in 
#setting the boundaries of the plot. To show this, let's plot our 
#`aoi_boundary_HARV` object with our vegetation plots.
...
#so that they are large enough to include all of the data included in all 
#of the layers. That's really handy!
