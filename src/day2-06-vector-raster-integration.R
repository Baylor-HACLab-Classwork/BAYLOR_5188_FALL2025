#Title: day2-06-vector-raster-integration.R
...
aoi_v <- vect(aoi_boundary_HARV)   # from sf
...
#convert to a data frame in order to plot using `ggplot`. To get the 
#boundary box from CHM, the `st_bbox()` will extract the 4 corners of the 
#rectangle that encompass all the features contained in this object. 
#The `st_as_sfc()` converts these 4 coordinates into a polygon that we can 
#plot:
...
#In the plot above, created in the challenge, all the vegetation plot 
#locations (black dots) appear on the Canopy Height Model raster layer 
#except for one. One is situated on the blank space to the left of the map. 
#Why?
...
#The extent can be created from a numeric vector (as shown above), 
#a matrix, or a list. 


#Once we have defined our new extent, we can use the `crop()` 
#function to crop our raster to this extent object.
...
#Now we can plot this cropped data. We will show the AOI boundary on the 
#same plot for scale.
...
#Often we want to extract values from a raster layer for particular 
...
#within our `aoi_boundary_HARV` polygon which surrounds the tower located 
#at the NEON Harvard Forest field site.

# tree_height <- terra::extract(x = CHM_HARV,
...
# 1) Make sure layer has a clean name for the output column
names(CHM_HARV) <- "chm"

# 2) Convert AOI to SpatVector and match CRS (you already did this)
aoi_v <- vect(aoi_boundary_HARV)
aoi_v <- project(aoi_v, CHM_HARV)

# 3) Extract ALL cell values that fall in each polygon
vals <- extract(CHM_HARV, aoi_v, cells = TRUE, xy = TRUE)
# cols: ID (polygon id), cell, x, y, chm

# 4) Drop NAs if needed
tree_height <- vals[!is.na(vals$chm), ]
...
ggplot() + 
  geom_histogram(data = tree_height, aes(x = chm)) +
...
# We can also use the
#`summary()` function to view descriptive statistics including min, max, 
#and mean height values. These values help us better understand vegetation 
#at our field site.
...
aoi_v <- vect(aoi_boundary_HARV)                 # accepts sf or sp, returns SpatVector
aoi_v <- project(aoi_v, CHM_HARV)                # reproject AOI to raster CRS (safe)

# per-polygon extraction of all cell values (list column); add `df=TRUE` for data.frame
vals <- extract(CHM_HARV, aoi_v, df = TRUE)

# or get a summary per polygon, e.g., mean tree height
mean_tree_height_AOI <- extract(CHM_HARV, aoi_v, fun = mean, na.rm = TRUE)
...
# 1) Give the layer a clean name (nice column name in outputs)
names(CHM_HARV) <- "chm"

# 2) Ensure the point is a SpatVector and in the same CRS as the raster
pt <- vect(point_HARV)              # works if point_HARV is sf or sp
pt <- project(pt, CHM_HARV)

# 3) Extract mean within a 20 m buffer around the point
mean_tree_height_tower <- extract(
  x   = CHM_HARV,
  y   = pt,
  buffer = 20,          # units = raster units (meters if your CHM is UTM)
  fun = mean,
  na.rm = TRUE
)
mean_tree_height_tower
# For a single point, the numeric mean is in the 'chm' column:
mean_tree_height_tower$chm[1]
...
#Because there are multiple plot locations, there will be multiple 
#averages returned, so the `df = TRUE` argument should be used.
...
pt <- vect(point_HARV)       # point_HARV can be sf or sp
pt <- project(pt, CHM_HARV)  # match CRS to the raster
# ensure same CRS as CHM

# 2) Mean within 20 m of each plot (terra returns a data.frame already)
#    bind=TRUE brings the plot attributes into the result row
mean_tree_height_plots_HARV <- extract(
  x      = CHM_HARV,
  y      = plots_v,
  buffer = 20,          # units = raster CRS (meters if UTM)
  fun    = mean,
  na.rm  = TRUE,
  bind   = TRUE
)
...
# 3) Convert to a plain data.frame for ggplot
df <- if (inherits(mean_tree_height_plots_HARV, "SpatVector")) {
  as.data.frame(mean_tree_height_plots_HARV)
} else if (inherits(mean_tree_height_plots_HARV, "sf")) {
  sf::st_drop_geometry(mean_tree_height_plots_HARV)
} else {
  mean_tree_height_plots_HARV
}

# 4) Choose an ID column (or create one)
if (!"siteID" %in% names(df)) {
  if ("plotID" %in% names(df)) {
    df$siteID <- df$plotID
  } else {
    df$siteID <- seq_len(nrow(df))
  }
}
...
#                                 df = TRUE)
 ...
 # mean_tree_height_plots_HARV
# drop missing CHM values and plot by plotID
df2 <- subset(df, !is.na(chm))

ggplot(df2, aes(x = reorder(plotID, chm), y = chm)) +
  geom_col() +
  coord_flip() +
  labs(
    title = "Mean Tree Height within 20 m of Each Plot",
    x = "Plot ID",
